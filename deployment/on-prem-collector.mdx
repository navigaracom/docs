...
COLLECTOR_API_TOKEN=<your-collector-api-token>

# Collector identity (optional — defaults to container hostname if not set)
# COLLECTOR_ID=collector-1

# LLM Configuration (OpenAI-compatible APIs only — see "LLM Configuration" section below for Vertex AI)
...
<Tabs>
  <Tab title="Google Vertex AI (Recommended)">
    ...
    **3. Authenticate with the service account:**
    ...
    Alternatively, mount the key file into the container and set the `GOOGLE_APPLICATION_CREDENTIALS` environment variable.
    ...
    **4. Set the following in your `.env` file:**
    ...
    # Option A: Host-level gcloud auth (if you ran gcloud auth above)
    # No additional config needed — the SDK picks up credentials automatically.

    # Option B: Service account key file mounted into the container
    # GOOGLE_APPLICATION_CREDENTIALS=/etc/navigara/vertex-ai-key.json
    ```

    If using Option B, add a volume mount in your `docker-compose.yml`:
...
| `COLLECTOR_API_TOKEN` | — | API token generated in Settings → API Tokens (required) |
...
| `LLM_PROVIDER` | `openai` | LLM provider: `openai` or `genai` |
...
services:
  collector-1:
    image: europe-west1-docker.pkg.dev/navigara-images/public/vision-collector:${NAVIGARA_VERSION}
...
| `authentication failed` | Check that `COLLECTOR_API_TOKEN` matches the token configured in Navigara |
